...
int dfs(Directory* node, int min, int target) {
    if (node->total_size > target && node->total_size < min) min = node->total_size;
    if (node->total_size > target) {
        for (int i = 0; i < (int)node->subfolders.size(); i++) {
            min = dfs(node->subfolders[i], min, target);
        }
    }
    return min;
}
...
int main() {
    ...
    // Output necessary deletion size
    int deletion_size = 30000000 - (70000000-root->total_size);
    cout << "To free up 30000000B, we need to delete " << deletion_size << endl;

    // Use DFS to find smallest file with available space
    int min = dfs(root, INT_MAX, deletion_size);
    cout << "The smallest subdirectory we can free contains " << min << endl;
